    ...
    function addEmptyAreaText(index) {
        if ($(index).has("div").length < 1) {
            if ($(index).hasClass("available-items-container")) {
                ...
                $(index).html("<span class='text-off empty-area-text'><?php echo lang('no_more_items_available'); ?></span>");
            } else {
                //if it's items sortable area
                $(index).html("<span class='text-off empty-area-text'><?php echo lang('drag_and_drop_items_here'); ?></span>");
            }
        }
    }
