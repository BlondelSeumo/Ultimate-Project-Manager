...
abstract class ParagonIE_Sodium_Core_Salsa20 extends ParagonIE_Sodium_Core_Util {
    ...
    public static function salsa20($len, $nonce, $key) {
        ...
        if ($len > 0) {
            $c .= self::substr(
                            self::core_salsa20($in, $kcopy, null), 0, $len
            );
        }
        try {
    ...
    public static function salsa20_xor_ic($m, $n, $ic, $k) {
        ...
        while ($mlen >= 64) {
            ...
            $c .= self::xorStrings(
                            self::substr($m, 0, 64), self::substr($block, 0, 64)
            );
            $u = 1;
            for ($i = 8; $i < 16; ++$i) {
                $u += self::chrToInt($in[$i]);
        ...
        if ($mlen) {
            $block = self::core_salsa20($in, $kcopy, null);
            $c .= self::xorStrings(
                            self::substr($m, 0, $mlen), self::substr($block, 0, $mlen)
            );
        }
        try {
    ...
    public static function salsa20_xor($message, $nonce, $key) {
        return self::xorStrings(
                        $message, self::salsa20(
                                self::strlen($message), $nonce, $key
                        )
        );
    }
