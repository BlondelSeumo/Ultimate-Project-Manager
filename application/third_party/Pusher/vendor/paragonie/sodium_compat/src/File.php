...
class ParagonIE_Sodium_File extends ParagonIE_Sodium_Core_Util {
    ...
    public static function box_seal($inputFile, $outputFile, $publicKey) {
        ...
        $msgKeypair = ParagonIE_Sodium_Compat::crypto_box_keypair_from_secretkey_and_publickey(
                        ParagonIE_Sodium_Compat::crypto_box_secretkey($ephKeypair), $publicKey
        );
        ...
        /** @var int $firstWrite */
        $firstWrite = fwrite(
                $ofp, $ephemeralPK, ParagonIE_Sodium_Compat::CRYPTO_BOX_PUBLICKEYBYTES
        );
    ...
    public static function box_seal_open($inputFile, $outputFile, $ecdhKeypair) {
        ...
        $msgKeypair = ParagonIE_Sodium_Compat::crypto_box_keypair_from_secretkey_and_publickey(
                        ParagonIE_Sodium_Compat::crypto_box_secretkey($ecdhKeypair), $ephemeralPK
        );
    ...
    protected static function box_encrypt($ifp, $ofp, $mlen, $nonce, $boxKeypair) {
        if (PHP_INT_SIZE === 4) {
            return self::secretbox_encrypt(
                            $ifp, $ofp, $mlen, $nonce, ParagonIE_Sodium_Crypto32::box_beforenm(
                                    ParagonIE_Sodium_Crypto32::box_secretkey($boxKeypair), ParagonIE_Sodium_Crypto32::box_publickey($boxKeypair)
                            )
            );
        }
        return self::secretbox_encrypt(
                        $ifp, $ofp, $mlen, $nonce, ParagonIE_Sodium_Crypto::box_beforenm(
                                ParagonIE_Sodium_Crypto::box_secretkey($boxKeypair), ParagonIE_Sodium_Crypto::box_publickey($boxKeypair)
                        )
        );
    }
    ...
    protected static function secretbox_encrypt($ifp, $ofp, $mlen, $nonce, $key) {
        ...
        $block0 = ParagonIE_Sodium_Core_Salsa20::salsa20_xor(
                        $block0, $realNonce, $subkey
        );

        $state = new ParagonIE_Sodium_Core_Poly1305_State(
                ParagonIE_Sodium_Core_Util::substr(
                        $block0, 0, ParagonIE_Sodium_Crypto::onetimeauth_poly1305_KEYBYTES
                )
        );
        ...
        $cBlock = ParagonIE_Sodium_Core_Util::substr(
                        $block0, ParagonIE_Sodium_Crypto::secretbox_xsalsa20poly1305_ZEROBYTES
        );
        $state->update($cBlock);
        fwrite($ofp, $cBlock);
        $mlen -= 32;
        ...
        while ($mlen > 0) {
            ...
            $cBlock = ParagonIE_Sodium_Core_Salsa20::salsa20_xor_ic(
                            $plaintext, $realNonce, $iter, $subkey
            );
    ...
    protected static function secretbox_decrypt($ifp, $ofp, $mlen, $nonce, $key) {
        ...
        fwrite(
                $ofp, self::xorStrings(
                        self::substr($block0, 32, $first32len), self::substr($first32, 0, $first32len)
                )
        );
        $mlen -= 32;
        ...
        while ($mlen > 0) {
            ...
            $pBlock = ParagonIE_Sodium_Core_Salsa20::salsa20_xor_ic(
                            $ciphertext, $realNonce, $iter, $subkey
            );
    ...
    public static function updateHashWithFile($hash, $fp, $size = 0) {
        ...
        for ($i = 0; $i < $size; $i += self::BUFFER_SIZE) {
            ...
            $message = fread(
                    $fp, ($size - $i) > self::BUFFER_SIZE ? $size - $i : self::BUFFER_SIZE
            );
