...
abstract class ParagonIE_Sodium_Core_Util {
    ...
    public static function bin2hex($binaryString) {
        ...
        for ($i = 0; $i < $len; ++$i) {
            ...
            $b = $chunk[1] >> 4;
            $hex .= pack(
                    'CC', (87 + $b + ((($b - 10) >> 8) & ~38)), (87 + $c + ((($c - 10) >> 8) & ~38))
            );
        }
        return $hex;
    }
    ...
    public static function bin2hexUpper($bin_string) {
        ...
        for ($i = 0; $i < $len; ++$i) {
             ...
             */
            $hex .= pack(
                    'CC', (55 + $b + ((($b - 10) >> 8) & ~6)), (55 + $c + ((($c - 10) >> 8) & ~6))
            );
        }
        return $hex;
    }
    ...
    public static function strlen($str) {
        ...
        return (int) (
                self::isMbStringOverride() ? mb_strlen($str, '8bit') : strlen($str)
                );
    }
    ...
    public static function verify_16($a, $b) {
        ...
        return self::hashEquals(
                        self::substr($a, 0, 16), self::substr($b, 0, 16)
        );
    }
    ...
    public static function verify_32($a, $b) {
        ...
        return self::hashEquals(
                        self::substr($a, 0, 32), self::substr($b, 0, 32)
        );
    }
